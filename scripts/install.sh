#!/bin/sh
#
# Herdr Desktop Linux installer.
#
# Installs the Herdr Desktop AppImage per-user (no root required): binary into
...
REPO="marcelormendes/herdr-desktop"
ASSET_NAME=""
DEFAULT_BASE_URL="https://github.com/${REPO}/releases/latest/download"
DEFAULT_ICON_URL="https://marcelormendes.github.io/herdr-desktop/icon.png"
VERSION=""
...
resolve_asset_name() {
  ...
  case "$(uname -m)" in
    x86_64) ASSET_NAME="herdr-desktop-linux-x86_64.AppImage" ;;
...
uninstall() {
  ...
  icon_file="$HOME/.local/share/icons/hicolor/1024x1024/apps/herdr-desktop.png"
  desktop_file="$HOME/.local/share/applications/herdr-desktop.desktop"
  removed=0
  for file in "${bin_dir}/herdr-desktop" "${icon_file}" "${desktop_file}"; do
  ...
  if [ "$removed" -eq 0 ]; then
    echo "Herdr Desktop is not installed; nothing to remove."
  fi
}
...
install_appimage() {
  ...
  staging="$bin_dir/.herdr-desktop.$$"
  mv -f "$tmpdir/${ASSET_NAME}" "$staging"
  chmod 0755 "$staging"
  mv -f "$staging" "$bin_dir/herdr-desktop"
  staging=""
  ...
  if curl -fSL --retry 2 --max-time 60 -o "$tmpdir/icon.png" "$ICON_URL" 2>/dev/null; then
    ...
    mkdir -p "$icon_dir"
    mv -f "$tmpdir/icon.png" "$icon_dir/herdr-desktop.png"
  else
...
warn_missing_herdr() {
  if ! command -v herdr >/dev/null 2>&1; then
    printf 'note: the herdr CLI was not found on PATH. Herdr Desktop needs it.\n' >&2
