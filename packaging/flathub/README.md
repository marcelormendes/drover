# Flathub submission — io.github.marcelormendes.herdr-desktop
...
1. **Host dependence** — Herdr Desktop's core functionality runs the host
...
5. **Pending real SHA** — the archive `sha256` in the manifest is currently a
   ...
   containing `herdr-desktop-linux-x64.zip` (v0.1.11+) publishes and its
...
- `io.github.marcelormendes.herdr-desktop.yml` — the manifest. It will pin the
...
1. Confirm the release includes `herdr-desktop-linux-x64.zip`.
2. In the manifest: set `url` to
   `…/releases/download/v<version>/herdr-desktop-linux-x64.zip`, copy the zip
...
3. Local test (requires flatpak tooling):
   ...
   flatpak-builder --user --install-deps-from=flathub build packaging/flathub/io.github.marcelormendes.herdr-desktop.yml
   flatpak-builder --user --install --force-clean build packaging/flathub/io.github.marcelormendes.herdr-desktop.yml
   flatpak run io.github.marcelormendes.herdr-desktop
   appstreamcli validate --no-net build/files/share/metainfo/io.github.marcelormendes.herdr-desktop.metainfo.xml
   ```
4. Submit: request the Flathub repository
   (`io.github.marcelormendes.herdr-desktop`) via the Flathub "New apps"
   process, then open a PR against `flathub/flathub` with
   `io.github.marcelormendes.herdr-desktop/` containing the manifest,
